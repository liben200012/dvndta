...
from DVNDTA import DVNDTA
from dataset_DVNDTA import GraphDataset, PLIDataLoader
...
if __name__ == '__main__':
    ...
    for repeat in range(repeats):
        ...
        train_dir = os.path.join(data_root, 'train_1')
        valid_dir = os.path.join(data_root, 'valid_1')
        test2013_dir = os.path.join(data_root, '2013')
        test2016_dir = os.path.join(data_root, '2016')
        test2019_dir = os.path.join(data_root, '2019')
        ...
        model = DVNDTA(35, 6, 256).to(device)
        ...
        for epoch in range(epochs):
            ...
            # start validating
            valid_rmse, valid_pr, valid_sd, valid_mae = val(model, valid_loader, device)
        ...
        valid_rmse, valid_pr, valid_sd, valid_mae = val(model, valid_loader, device)
        test2013_rmse, test2013_pr, test2013_sd, test2013_mae = val(model, test2013_loader, device)
        test2016_rmse, test2016_pr, test2016_sd, test2016_mae = val(model, test2016_loader, device)
        test2019_rmse, test2019_pr, test2019_sd, test2019_mae = val(model, test2019_loader, device)

        msg = "valid_rmse-%.4f, valid_pr-%.4f,|| test2013_rmse-%.4f, test2013_pr-%.4f, test2013_sd-%.4f, test2013_mae-%.4f,|| test2016_rmse-%.4f, test2016_pr-%.4f,test2016_sd-%.4f, test2016_mae-%.4f,|| test2019_rmse-%.4f, test2019_pr-%.4f,test2019_sd-%.4f, test2019_mae-%.4f" \
              % (
              valid_rmse, valid_pr, test2013_rmse, test2013_pr, test2013_sd, test2013_mae, test2016_rmse, test2016_pr,test2016_sd, test2016_mae, test2019_rmse, test2019_pr,test2019_sd, test2019_mae)
